...
```mermaid
graph TD
    subgraph "第一阶段：数据上传与预处理"
        A[用户上传文件] --> B{文件类型判断}
        B -->|非结构化<br/>PDF/DOCX/TXT| C[存入 File/Unstructured/<类目>]
        B -->|结构化<br/>CSV/XLSX| D[存入 File/Structured/<数据表名>]
        
        D --> E[Pandas 读取表格]
        E --> F[行级序列化转换<br/>格式: 【字段:值, ...】]
        F --> G[保存为 .txt 文本]
        G --> H[删除原始 Excel/CSV 文件]
    end

    subgraph "第二阶段：向量知识库构建"
        I[用户创建知识库] --> J{选择数据源}
        J -->|非结构化| K[SimpleDirectoryReader 加载文档]
        J -->|结构化| L[按行分割文本<br/>创建 TextNode 节点]
        
        K --> M[VectorStoreIndex.from_documents]
        L --> N["VectorStoreIndex(nodes)"]
        
        M & N --> O[DashScope Embedding 向量化]
        O --> P[FAISS 索引构建]
        P --> Q[持久化存储至 VectorStore/<库名>]
    end

    subgraph "第三阶段：RAG 问答流程"
        R[用户输入问题 + 可选临时文件] --> S{是否有临时文件?}
        S -->|是| T[实时构建临时知识库 tmp_abcd]
        S -->|否| U[加载指定持久化知识库]
        
        T & U --> V[向量检索 Retriever<br/>Top-K=20 粗排]
        V --> W[DashScope Rerank 重排序<br/>精排 Top-N]
        
        W --> X{相似度阈值过滤}
        X -->|Score >= Threshold| Y[构建上下文 Context]
        X -->|Score < Threshold| Z[丢弃低相关片段]
        
        Y --> AA[组装 Prompt:<br/>参考内容 + 用户问题]
        AA --> AB[调用 Qwen-Max/Plus LLM]
        AB --> AC[流式输出回答 + 展示召回原文]
    end
